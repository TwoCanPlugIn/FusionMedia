...
MediaPlayerControlWindow::MediaPlayerControlWindow( wxWindow* parent, wxEvtHandler *handler) : MediaPlayerControlWindowBase(parent) {
	
	// Set the dialog's icon
	wxIcon icon;
	//icon.CopyFromBitmap(*_32);
	//MediaPlayerControlWindow::SetIcon(icon);
...
void MediaPlayerControlWindow::Initialize(void) {
	...
	stopWatch->Connect(stopWatch->GetId(), wxEVT_TIMER, wxTimerEventHandler(MediaPlayerControlWindow::OnTimer), NULL, this);
	ResetTimer();
...
void MediaPlayerControlWindow::OnClose(wxCloseEvent& event) {
	if (stopWatch->IsRunning()) {
		stopWatch->Stop();
	}

	stopWatch->Disconnect(stopWatch->GetId(), wxEVT_TIMER, wxTimerEventHandler(MediaPlayerControlWindow::OnTimer));
	...
	wxCommandEvent *closeEvent = new wxCommandEvent(wxEVT_MEDIAPLAYERCONTROL_DIALOG_EVENT, DIALOG_CLOSED);
...
void MediaPlayerControlWindow::OnTimer(wxTimerEvent& event) {
...
void MediaPlayerControlWindow::OnReset(wxCommandEvent &event) {
	if (stopWatch->IsRunning()) {
		stopWatch->Stop();
	}
	ResetTimer();
...
void MediaPlayerControlWindow::OnStart(wxCommandEvent &event) {
...
void MediaPlayerControlWindow::OnStarboard(wxCommandEvent &event) {
	// Notify parent to drop a waypoint at the starboard end
	wxCommandEvent *commandEvent = new wxCommandEvent(wxEVT_MEDIAPLAYERCONTROL_DIALOG_EVENT, DIALOG_ON);
	wxQueueEvent(eventHandlerAddress, commandEvent);
}

void MediaPlayerControlWindow::OnPort(wxCommandEvent &event) {
	wxCommandEvent *commandEvent = new wxCommandEvent(wxEVT_MEDIAPLAYERCONTROL_DIALOG_EVENT, DIALOG_ON);
	wxQueueEvent(eventHandlerAddress, commandEvent);
...
void MediaPlayerControlWindow::OnCancel(wxCommandEvent &event) {
	this->Close();
}

void MediaPlayerControlWindow::ResetTimer(void) {
...
bool MediaPlayerControlWindow::CalculateIntersection(double latitude1, double longitude1, double  bearing1, double latitude2, double longitude2, double bearing2, double *latitude3, double *longitude3) {
 ...
 // see www.edwilliams.org/avform.htm#Intersection
	...
	*longitude3 = lon3 * 180 / M_PI;
	...
	//
	double MediaPlayerControlWindow::BearingBetweenPoints(double lat1, double lon1, double lat2, double lon2) {
...
double MediaPlayerControlWindow::HaversineFormula(double latutude1, double longitude1, double latitude2, double longitude2) {
...
double MediaPlayerControlWindow::SphericalCosines(double latitude1, double longitude1, double latitude2, double longitude2) {
...
double	MediaPlayerControlWindow::Bearing(double latitude1, double longitude1, double latitude2, double longitude2) {
...
bool MediaPlayerControlWindow::IsWithinBoundingBox(double latitude, double longitude) {
